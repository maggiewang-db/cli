...
func TestDockerHostCommandReportsUnsupportedWorkspaceProfile(t *testing.T) {
	t.Parallel()

	p := dockerHostTestProfile()
	p.Name = "e2-dogfood"
	p.Host = "https://dogfood.staging.databricks.test"
	p.AccountID = "account-id"
	deps := dockerHostTestDeps(t, p)
	deps.validateWorkspaceHost = dockercredentials.ValidateWorkspaceHost
	deps.newWorkspaceClient = func(*databricks.Config) (*databricks.WorkspaceClient, error) {
		t.Fatal("newWorkspaceClient should not be called")
		return nil, nil
	}

	_, err := executeDockerHostCommand(t.Context(), deps, "--profile", "e2-dogfood")
