...
func TestConfigureDockerCommandRewritesInvalidRefreshTokenErrorBeforeMutation(t *testing.T) {
	ctx := cmdio.MockDiscard(t.Context())
	dockerDir := t.TempDir()
...
func TestConfigureDockerCommandReportsUnsupportedWorkspaceProfileBeforeWorkspaceRequestOrMutation(t *testing.T) {
	ctx := cmdio.MockDiscard(t.Context())
	dir := t.TempDir()
	configFile := filepath.Join(dir, ".databrickscfg")
	dockerDir := filepath.Join(dir, "docker")
	...
	writeConfigureDockerProfile(t, ctx, configFile, &config.Config{
		Profile:     "e2-dogfood",
		Host:        "https://dogfood.staging.databricks.test",
	...
	cmd := newDockerConfigureTestCommandWithDeps(ctx, deps, "docker", "configure", "e2-dogfood")
	err = cmd.Execute()
	assert.EqualError(t, err, `profile "e2-dogfood" does not target a supported Databricks workspace host. Select a workspace profile, or create one with databricks auth login --host <workspace-url> --profile <name>`)
