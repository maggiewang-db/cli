...
// loadAndValidateDockerProfile verifies that the selected profile is a workspace OAuth profile supported by the helper.
...
// resolveDockerWorkspaceID clears the CLI-only "none" sentinel before asking the workspace API for an ID.
